...
##
## Publication outputs: SCAM residual diagnostics, Fig. 3h SCAM vs GAM, Fig. 3i
## effect forest, and leave-one-out SCAM vs GAM panels (section 3).
## DCR / ORR / durable-benefit panels (Fig. 3a–c) are written by src/DCR_ORR_DB.R.
##
...
source(file.path(SCRIPT_DIR, "cochran_armitage_trend.R"))
setup_arial_font()
...
## Input workbook has multiple sheets; the drug-response table lives in this sheet (first sheet is README).
...
## Shared reader for the input workbook: reads the drug-response sheet and standardizes
## column names (spaces -> "_", uppercased). The sheet has header collisions after
## uppercasing (e.g. Approved/approved, On_label/on label, and blank "Unnamed" columns);
## keep the first occurrence of each name so the canonical columns win and tibble name
## repair does not error.
read_input_sheet <- function(path, sheet = DEFAULT_INPUT_SHEET) {
  df_raw <- readxl::read_xlsx(path, sheet = sheet)
  new_names <- toupper(stringr::str_replace_all(names(df_raw), "\\s+", "_"))
  keep_cols <- !duplicated(new_names)
  df_local <- df_raw[, keep_cols, drop = FALSE]
  names(df_local) <- new_names[keep_cols]
  ## Always drop chemo rows (On_label == "chemo") from every analysis in this file.
  ## In the KM plotting scripts chemo maps to the SC (standard-of-care) reference group,
  ## but the data-modeling analyses here never use SC, so chemo is excluded globally.
  if ("ON_LABEL" %in% names(df_local)) {
    is_chemo <- !is.na(df_local[["ON_LABEL"]]) & toupper(trimws(as.character(df_local[["ON_LABEL"]]))) == "CHEMO"
    if (any(is_chemo)) {
      message("read_input_sheet: excluded ", sum(is_chemo), " chemo row(s) (On_label == 'chemo').")
      df_local <- df_local[!is_chemo, , drop = FALSE]
    }
  }
  df_local
}
...
fmt_p_plot <- function(p) {
  if (length(p) == 0L) return(character(0))
  vapply(p, function(x) {
    if (is.na(x)) return(plain_plot_label_to_plotmath("p = NA"))
    rhs <- trimws(format.pval(x, digits = 3, eps = 1e-4))
    if (grepl("^<", rhs)) {
      return(paste0("italic(p) < ", trimws(sub("^<\\s*", "", rhs))))
    }
    paste0("italic(p) == ", num_str_to_plotmath_rhs(rhs))
...
df <- run_step("Step 2: Read and standardize data", function() {
  df_local <- read_input_sheet(input_path)

  required_candidates <- list(
    REPORT_ID = c("REPORT_ID", "PATIENT", "PATIENT_ID", "SAMPLE_ID", "MODEL", "MODEL_ID", "PCT_ID"),
    COMPOUND = c("COMPOUND", "DRUG", "COMPOUND_NAME"),
    SCORE = c("LEVEL", "SCORE", "PREDICTIVE_SCORE", "PRED_SCORE", "DDA_SCORE"),
    TIME_TO_DOUBLE = c(RESPONSE_COL_NAME, "TIMETODOUBLE", "TIME_TO_DOUBLE", "TIME_TO_DOUBLING", "TTD", "TIMETODOUBLE_(DAYS)")
  )
  ...
  has_response_best <- any(c("RESPONSE_BEST_RESPONSE", "RESPONSE_BESTRESPONSE") %in% colnames(df_local))
  best_resp_col <- intersect(c("RESPONSE_BEST_RESPONSE", "RESPONSE_BESTRESPONSE"), colnames(df_local))[1]
  df_local <- df_local %>%
    transmute(
      ...
      ON_LABEL = if ("ON_LABEL" %in% colnames(df_local)) as.character(.data[["ON_LABEL"]]) else NA_character_,
      RESPONSE_BESTAVGRESPONSE = if (has_response) as.character(.data[["RESPONSE_BESTAVGRESPONSE"]]) else NA_character_,
      RESPONSE_BEST_RESPONSE = if (has_response_best) as.character(.data[[best_resp_col]]) else NA_character_
    ) %>%
    filter(!is.na(REPORT_ID), !is.na(COMPOUND), !is.na(SCORE), !is.na(TIME_TO_DOUBLE))

  if (has_response_best) {
    log_note("Optional column RESPONSE_BEST_RESPONSE (e.g. Excel: Response_Best_Response / Response BestResponse) retained in df / df_all_ranks for mRECIST Best_Response panels.")
  }
...
run_step("Step 9: Publication-quality figures (peer review & PI perspective)", function() {
  ...
  save_fig(p22_ox_purple, file.path(fig_dir, "Fig3h_SCAM_vs_GAM_all_ranks.png"), FIG_F22B_WIDTH, FIG_F22B_HEIGHT)
  save_fig(p8, file.path(fig_dir, "Fig3i_F8_Effect_forest_enhanced.png"), FIG_F8_WIDTH, FIG_F8_HEIGHT)
  ## Fig. 3a–c (DCR / ORR / durable benefit) are saved by src/DCR_ORR_DB.R.
...
input_path <- file.path(DATA_DIR, DEFAULT_INPUT_XLSX)
output_root <- file.path(DATA_MODELING_OUTPUT_ROOT, "leave_one_out")
date <- format(Sys.Date(), "%Y_%m_%d")
...
EFIG2_LEAVE_OUT_PREFIX <- c(
  leave_out_baseline_ALL = "EFig2a_",
  leave_out_CDK4_6 = "EFig2b_",
  leave_out_MAP2K1_2 = "EFig2c_",
  leave_out_panPI3K = "EFig2d_"
)

save_f22b_plot <- function(plot_obj, fig_w, fig_h, dpi = 300L) {
  tag <- f22b_leave_out_filename_tag(left_out_target)
  prefix <- unname(EFIG2_LEAVE_OUT_PREFIX[tag])
  if (is.na(prefix)) prefix <- ""
  base <- paste0(prefix, "SCAM_vs_GAM_", tag)
  collect_dir <- file.path(lou_date_dir, "SCAM_vs_GAM_figures")
...
lou_f22b_plot_title <- function(target_key) {
  ...
  mapped <- lookup_map_target(key)
  if (identical(mapped, "MAP2K1/2") || identical(key, "MAP2K1/2") ||
      identical(mapped, "MAP2K1") || identical(key, "MAP2K1")) return("MAP2K1/2")
  mapped
}
...
build_f22b_scam_gam_plot <- function(
  ...
  ## Keep rotated n-labels inside the panel: high points (e.g. CDK rank 1) otherwise
  ## push the closing ')' into the top margin where it clips / separates from the digits.
  n_headroom <- 0.14 * y_span
  obs_lab <- obs_rank %>%
    dplyr::mutate(
      y_nlab = mean_ttd + 1.96 * se_ttd + y_lab_off,
      y_nlab = pmin(y_nlab, y_hi - n_headroom)
    )
  ggplot2::ggplot(obs_rank, ggplot2::aes(x = factor(rank_int, levels = rank_levels, labels = rank_labels), y = mean_ttd)) +
    ...
    ## Draw n-labels last so ribbons/lines never cover glyphs; hjust=0 keeps text
    ## closer to the point so high ranks stay clear of the panel top.
    ggplot2::geom_text(
      ggplot2::aes(
      ...
      data = obs_lab, angle = 90, hjust = 0, vjust = 0.5, size = n_label_sz, color = "black"
    ) +
